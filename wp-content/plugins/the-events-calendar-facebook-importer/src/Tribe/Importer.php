...
class Tribe__Events__Facebook__Importer {
	...
	public function reset_default_app() {
		...
		if ( $this->get_app_id() == self::$default_app_id ){
			$reset_default = true;
			Tribe__Events__Main::setOption( 'fb_api_key', '' );
		}
		if ( $this->get_app_secret() == self::$default_app_secret ){
			$reset_default = true;
			Tribe__Events__Main::setOption( 'fb_api_secret', '' );
		}
		if ( $reset_default ){
	...
	public function empty_app_notices() {
		...
		printf( esc_html__( 'Signing up for a Facebook App ID and Secret only takes a second. Visit %s to create yours. ', 'tribe-fb-import' ), '<a href="https://developers.facebook.com/apps">https://developers.facebook.com/apps</a>' );
		printf( esc_html__( 'Visit %s to learn more about Facebook App Ids and Secrets. ', 'tribe-fb-import' ), '<a href="https://developers.facebook.com/docs/guides/canvas/">https://developers.facebook.com/docs/guides/canvas/</a>' );
		printf( esc_html__( 'You must enter a Facebook App ID and App Secret to continue importing events from Facebook. Visit %s to create yours. ', 'tribe-fb-import' ), '<a href="https://developers.facebook.com/apps">https://developers.facebook.com/apps</a>' );
		echo '</p></div>';
	}
	...
	public function get_app_id() {
		return apply_filters( 'tribe_fb_get_app_id', Tribe__Events__Main::getOption( 'fb_api_key' ) );
	}
	...
	public function get_app_secret() {
		return apply_filters( 'tribe_fb_get_app_secret', Tribe__Events__Main::getOption( 'fb_api_secret' ) );
	}
	...
	public function get_user_or_page_ids() {
		$user_or_page_ids = Tribe__Events__Main::getOption( 'fb_user_or_page_ids', array() );
	 ...
	 */
	public function json_retrieve_data( $url ) {
	...
	public function parse_facebook_venue( $facebook_event ) {
		...
		if ( empty( $facebook_event->venue ) && empty( $facebook_event->location ) ){
			...
			$event_place = $this->json_retrieve( $url );
	...
	public function parse_facebook_event( $facebook_event ) {
		...
		$event_params = array(
			...
			'post_status' => Tribe__Events__Main::getOption( 'fb_default_status', 'publish' ),
		...
		if ( $facebook_event->all_day ) {
			$event_params['EventStartDate'] = tribe_event_beginning_of_day( $start_time );
			$event_params['EventEndDate']   = tribe_event_end_of_day( $start_time );
		} else {
			$event_params['EventStartDate'] = Tribe__Events__Date_Utils::date_only( $start_time );
			$event_params['EventEndDate']   = Tribe__Events__Date_Utils::date_only( $end_time );
		}
		...
		if ( $facebook_event->all_day ) {
			...
			$event_params['EventStartHour'] = Tribe__Events__Date_Utils::hour_only( $start_time );
			$event_params['EventStartMinute'] = Tribe__Events__Date_Utils::minutes_only( $start_time );
			$event_params['EventStartMeridian'] = Tribe__Events__Date_Utils::meridian_only( $start_time );
			$event_params['EventEndHour'] = Tribe__Events__Date_Utils::hour_only( $end_time );
			$event_params['EventEndMinute'] = Tribe__Events__Date_Utils::minutes_only( $end_time );
			$event_params['EventEndMeridian'] = Tribe__Events__Date_Utils::meridian_only( $end_time );
		}
	 ...
	 * "unexpected" results when retrieving the hour, etc, via Tribe__Events__Date_Utils.
	 ...
	 * @since 1.0
	 */
	public function create_local_event( $facebook_event_id ) {
		// If the event has already been imported then report an error and skip
		if ( $this->find_local_object_with_fb_id( $facebook_event_id, 'event' ) ) {
	...
	protected function is_midnight( $time ) {
		try {
			...
			$time = $time->format( Tribe__Events__Date_Utils::TIMEFORMAT );
	...
	public function add_import_page() {
		add_submenu_page( '/edit.php?post_type=' .Tribe__Events__Main::POSTTYPE, __( 'Import: Facebook', 'tribe-fb-import' ), __( 'Import: Facebook', 'tribe-fb-import' ), 'edit_posts', 'import-fb-events', array( $this, 'do_import_page' ) );
	}
	...
	public function add_setting_page() {
		 ...
		 * @var $facebook_tab
		 */
		include( self::$plugin_root . 'src/admin-views/settings-tab.php' );
		new Tribe__Events__Settings_Tab( 'fb-import', __( 'Facebook', 'tribe-fb-import' ), $facebook_tab );
	}
	...
	public function do_auto_import() {
		...
		$fb_uids = $this->parse_fb_uids( Tribe__Events__Main::getOption( 'fb_uids' ) );
	...
	public function manage_import_schedule() {
		...
		if ( Tribe__Events__Main::getOption( 'fb_auto_import' ) ) {
			$recurrence = Tribe__Events__Main::getOption( 'fb_auto_frequency' );
	...
	public function addFacebookToolbarItems() {
		...
		if ( current_user_can( 'publish_tribe_events' ) ) {
			$wp_admin_bar->add_menu( array(
				...
				'href' => trailingslashit( get_admin_url() ) . 'edit.php?post_type=' .Tribe__Events__Main::POSTTYPE . '&page=import-fb-events',
		...
		if ( current_user_can( 'manage_options' ) ) {
			$wp_admin_bar->add_menu( array(
				...
				'href' => trailingslashit( get_admin_url() ) . 'edit.php?post_type=' .Tribe__Events__Main::POSTTYPE . '&page=tribe-events-calendar&tab=fb-import',
	...
	public function addLinksToPluginActions( $actions ) {
		if ( class_exists( Tribe__Events__Main ) ) {
			...
			$actions['settings'] = '<a href="' . add_query_arg( $query_args, admin_url( 'edit.php' ) ) .'">' . __( 'Settings', 'tribe-fb-import' ) . '</a>';
		}
		return $actions;
	}
