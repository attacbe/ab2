...
$facebook_tab = array(
	...
	'fields' => array(
		'fb-import-instructions' => array(
			...
			'html' =>
				'<div id="modern-tribe-info">
					<h2>' . esc_html__( 'Import Facebook Events', 'tribe-fb-import' ) . '</h2>
					<h3>' . esc_html__( 'Getting Started', 'tribe-fb-import' ) . '</h3>
					<p>' . esc_html__( 'You need a Facebook App ID and App Secret to access data via the Facebook Graph API to import your events from Facebook.', 'tribe-fb-import' ) . '</p>
					<ul class="admin-list">
						<li>' . sprintf( esc_html__( '%s to learn more about Facebook Apps', 'tribe-fb-import' ), '<a href="https://developers.facebook.com/docs/web/gettingstarted/" target="_blank">' . __( 'Click here', 'tribe-fb-import' ) . '</a>' ) . '</li>
						<li>' . sprintf( esc_html__( '%s to view or create Facebook Apps', 'tribe-fb-import' ), '<a href="https://developers.facebook.com/apps" target="_blank">' . esc_html__( 'Click here', 'tribe-fb-import' ) . '</a>' ) . '</li>
					</ul>
					<h3>' . esc_html__( 'Selecting pages or organizations to import events with', 'tribe-fb-import' ) . '</h3>
					<p>' . esc_html__( 'You can retrieve and import events belonging to a Facebook organization or a Facebook page. You will need the username(s) or ID of each organization or page that you want to fetch events from. We do not currently support retrieving events from personal profiles. If you want to import an event from an individual, you can do that with the event ID on the', 'tribe-fb-import' ) . ' <a href="' . esc_url( get_admin_url() ) . 'edit.php?post_type=tribe_events&page=import-fb-events">' . esc_html__( 'Import: Facebook page', 'tribe-fb-import' ) . '</a></p>
		...
		'tribeEventsFacebookAcctTitle' => array(
			'type' => 'html',
			'html' => '<h3>' . __( 'Account Settings', 'tribe-fb-import' ) . '</h3>',
		),
		'fb_api_key' => array(
			'type' => 'text',
		...
		'tribeEventsFacebookImportTitle' => array(
			'type' => 'html',
			'html' => '<h3>' . __( 'Import Settings', 'tribe-fb-import' ) . '</h3>',
		),
