...
* Fixed a bug where an event could import with the date one day early (thanks to rebatch on the forums for the heads up!)
* Fixed a bug where imported images used same filename causing them to overwrite each other (thanks to honeyl on the forums for reporting this!)
...
* Fix a bug causing events without a specific end time to be imported as all day events (our thanks to Christian Kramer	for highlighting this)
...
* Corrected textdomains for proper translations

= 3.6 =

* Fixed markup in the import form (thanks to both Malisa at ManageWP and geekteq)
